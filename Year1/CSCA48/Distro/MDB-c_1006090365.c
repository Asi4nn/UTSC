 ...
 *
 * Student Name (Last name, First name): Wang, Si
 * Student Number: 1006090365
 * UTORid: wangsi97
 * Your instructor's name is: Brian Harrington
 */
...
{
    ReviewNode *node = (ReviewNode *)calloc(1, sizeof(ReviewNode));
    // if something goes wrong allocating the memory, return NULL
	if (node == NULL)
	{
		return NULL;
	}
	...
	{
		strcpy(node -> review.movie_title, title);
		strcpy(node -> review.movie_studio, studio);
		node -> review.year = year;
		node -> review.BO_total = BO_total;
		node -> review.score = score;
		node -> review.cast = NULL;
		node -> next = NULL;
...
ReviewNode *findMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
    ...
    while (head != NULL)
	{
        // check if the title, studio, and year are the same
...
ReviewNode *insertMovieReview(char *title, char *studio, int year, double BO_total, int score,
                              ReviewNode *head)
{
    // check if the given movie already exists
	if (findMovieReview(title, studio, year, head) == NULL)
	{
        // insert the new node at head
		ReviewNode *new_node = newMovieReviewNode(title, studio, year, BO_total, score);
		new_node -> next = head;
		head = new_node;
	}
...
{
    int counter = 0;
    // loop through the list, adding 1 each time
	while (head != NULL)
	{
		counter++;
		head = head -> next;
...
void updateMovieReview(char *title, char *studio, int year, double BO_total, int score,
                       ReviewNode *head)
{
    // check valid score input for score
	if (score >= 0 && score <= 100)
	{
        // check if the given movie is in the list
		if (findMovieReview(title, studio, year, head) != NULL)
		{
            // edit the given movie
			ReviewNode *review = findMovieReview(title, studio, year, head);
			review -> review.BO_total = BO_total;
...
ReviewNode *deleteMovieReview(char *title, char *studio, int year, ReviewNode *head)
{
	...
	ReviewNode *leader = head;
	ReviewNode *delete_review = findMovieReview(title, studio, year, head); // pointer to the review to be deleted
    // check if it's in the list
	if (delete_review != NULL)
	{
        // check if head is the one to delete
		if (leader != delete_review)
		{
            // loop through until leader == delete_review
			while (leader != delete_review && leader != NULL)
			{
				follower = leader;
				leader = leader -> next;
			}
...
{
	double BO_overall = 0; // accumulator
    // loop through the list
    while (head != NULL)
	{
		BO_overall += head -> review.BO_total;
...
double queryReviewsByStudio(char *studio, ReviewNode *head)
{
	...
	while (head != NULL)
	{
        // check if the current movie has the given studio
...
double queryReviewsByScore(int min_score, ReviewNode *head)
{
	...
	while (head != NULL)
	{
        // check if the current movie score is above the given min_score
...
{
    ReviewNode *follower = NULL;
    int num_of_reviews = countReviews(head);

    // i'm using bubble sort to sort the list
    for (int i = 0; i < num_of_reviews-1; i++)
    {
        for (int j = 0; j < num_of_reviews-1-i; j++)
        {
            ReviewNode *iterator_head = head;
            int k = 0;
            // iterate to the jth index in the list
            for (k = 0; k < j; k++)
            {
                follower = iterator_head;
                iterator_head = iterator_head -> next;
            }
            // check if the two adjacent nodes are sorted
            if (strcmp(iterator_head -> review.movie_title, iterator_head -> next -> review.movie_title) > 0)
            {
                // swaps the nodes if they are not in order

                // swaps for the case of the first and second indices
                if (k == 0 && iterator_head -> next != NULL)
                {
                    ReviewNode *temp  = iterator_head -> next;
                    iterator_head -> next = iterator_head -> next -> next;
                    temp -> next = head;
                    head = temp;
                }
                // swaps for all other cases
                else
                {
                    ReviewNode *temp  = iterator_head -> next;
                    iterator_head -> next = iterator_head -> next -> next;
                    temp -> next = iterator_head;
                    follower -> next = temp;
                }
            }
        }
...
{
    ReviewNode *insert_review = findMovieReview(title, studio, year, head); // pointer to the review to insert to member to

    // check if the given review exists
    if (insert_review != NULL)
    {
        CastList *new_node = (CastList *)calloc(1, sizeof(CastList)); // create new cast last node
        // make new_node the head for review.cast with the given name
        strcpy(new_node -> name, name);
        new_node -> next = insert_review -> review.cast;
        insert_review -> review.cast = new_node;
    }
}
...
int countNames(MovieReview *movie, char *name)
{
    ...
    CastList *head = movie -> cast;
    // loop through the cast list
    while (head != NULL)
    {
        // check if the substring name is in each member's name
        if (strstr(head -> name, name) != NULL)
        {
            // add 1 if name is within the member's name
            counter++;
        }
        head = head -> next;
    }
