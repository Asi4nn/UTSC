...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
	...
	if (root -> bar == bar && root -> index == index)
	{
		...
		{
            new = find_successor(root -> right);
            root -> key = new -> key;
            root -> bar = new -> bar;
            root -> index = new -> index;
            root -> freq = new -> freq;
            root -> right = BST_delete(root -> right, new -> bar, new -> index);
		}
	}
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
     if (root == NULL) // return if the first BST node is null
     {
         return;
     }

     int i = 0;
     while (i < 100 && strcmp(note_src, note_names[i]) != 0)
     {
         i++; // get the index of the note_src
     }

     if (root -> freq == note_freq[i]) // check if the current node freq is equal to note_src
     {
         int j = 0;
         while (j < 100 && strcmp(note_dst, note_names[j]) != 0)
         {
             j++;
         }
         root -> freq = note_freq[j];
     }
...
BST_Node *BST_insert_dupe(BST_Node *root, BST_Node *new_node)
{
     /*
      * This function inserts a new node into the BST. The
      * node must already have been initialized with valid
 	...
 	if (root -> key == new_node -> key)
 	{
        while (BST_search(root, new_node -> bar, new_node -> index) != NULL)
        {
            if (new_node -> index == 0)
            {
                new_node -> index += 0.000001;
                new_node -> key = 10*(new_node -> bar) + (new_node -> index);
            }
        ...
        root = BST_insert_dupe(root, new_node);
 	}
 	else if (new_node -> key > root -> key)
 	{
 		root -> right = BST_insert_dupe(root -> right, new_node);
 	}
 	else // if (new_node -> key < root -> key)
 	{
 		root -> left = BST_insert_dupe(root -> left, new_node);
 	}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     ****/
     ...
     BST_Node *new_root = newBST_Node(root -> freq, root -> bar, root -> index);

     if ((root -> index + time_shift < 1 && root -> index + time_shift >= 0) || root -> index == 1)
     {
         int i = 0;
         while (i < 100 && note_freq[i] != root -> freq)
         {
             i++;
         }
         if (i != 100)
         {
             ...
             if (i >= 0 && i < 100)
             {
                 BST_Node *new = newBST_Node(note_freq[i], root -> bar, root -> index + time_shift);
                 BST_insert_dupe(new_root, new);
             }
         }
     }
     ...
     while (left != NULL)
     {
         if ((left -> index + time_shift < 1 && left -> index + time_shift >= 0) || right -> index == 1)
         {
             int i = 0;
             while (i < 100 && note_freq[i] != left -> freq)
             {
                 i++;
             }
             if (i != 100)
             {
                 ...
                 if (i >= 0 && i < 100)
                 {
                     BST_Node *new = newBST_Node(note_freq[i], left -> bar, left -> index + time_shift);
                     BST_Node *old = newBST_Node(left -> freq, left -> bar, left -> index);
                     BST_insert_dupe(new_root, old);
                     BST_insert_dupe(new_root, new);
                 }
             }
         }
     ...
     while (right != NULL)
     {
         if ((right -> index + time_shift < 1 && right -> index + time_shift >= 0) || right -> index == 1)
         {
             int i = 0;
             while (i < 100 && note_freq[i] != right -> freq)
             {
                 i++;
             }
             if (i != 100)
             {
                 ...
                 if (i >= 0 && i < 100)
                 {
                     BST_Node *new = newBST_Node(note_freq[i], right -> bar, right -> index + time_shift);
                     BST_Node *old = newBST_Node(right -> freq, right -> bar, right -> index);
                     BST_insert_dupe(new_root, old);
                     BST_insert_dupe(new_root, new);
                 }
             }
