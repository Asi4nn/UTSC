...
const ADT CORRECT_ADT = STACK; /** PICK: STACK or QUEUE */
...
bool enqueue(Data *data, char *name)
{
    ...
    Ticket *new_ticket = (Ticket *)calloc(1, sizeof(Ticket));
    if (new_ticket == NULL)
    {
        return false;
    }
    ...
    if (data -> first == NULL)
    {
        data -> first = new_ticket;
        data -> last = new_ticket;
    }
    else
    {
        data -> last -> next = new_ticket; // problem
